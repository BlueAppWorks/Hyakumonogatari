...
BEGIN
    ...
    DROP SERVICE IF EXISTS app_services.hyakumonogatari;
...
BEGIN
    ...
    BEGIN SELECT value INTO :db_name FROM app_config.settings WHERE key = 'db_name';
    EXCEPTION WHEN OTHER THEN NULL; END;
    db_port := COALESCE(:db_port, '5432');
    db_name := COALESCE(:db_name, 'postgres');
    ...
    create_sql := 'CREATE SERVICE IF NOT EXISTS app_services.hyakumonogatari '
    ...
    BEGIN
        ALTER SERVICE IF EXISTS app_services.hyakumonogatari RESUME;
    EXCEPTION WHEN OTHER THEN NULL; END;
...
$$
BEGIN
    SHOW SERVICES LIKE 'hyakumonogatari' IN SCHEMA app_services;
...
$$
BEGIN
    SHOW ENDPOINTS IN SERVICE app_services.hyakumonogatari;
...
BEGIN
    BEGIN
        SELECT SYSTEM$GET_SERVICE_LOGS(
            'app_services.hyakumonogatari', 0,
            'hyakumonogatari', :num_lines
        ) INTO :log_result;
    EXCEPTION WHEN OTHER THEN
