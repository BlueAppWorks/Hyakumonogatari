...
EAI_REF_NAME = "external_access"
EAI_DISPLAY_LABEL = "External Access (PostgreSQL & OpenAI)"
...
# Resource type label shown in EAI guidance
...
# ============================================================
# Page: Setup (4-step guided wizard)
# ============================================================
    ...
    # ----------------------------------------------------------
    # Step 2: Database Connection (with OpenAI API Key)
    # ----------------------------------------------------------
    s2 = step_state(2, step2_done)
    st.markdown(
        _step_header(2, f"{RESOURCE_TYPE_LABEL} & OpenAI Connection", s2),
        unsafe_allow_html=True,
    )

    with st.expander(
        f"Configure {RESOURCE_TYPE_LABEL} and OpenAI connection",
        expanded=(s2 == "current"),
    ):
        if not step1_done:
            ...
            db_name_display = get_setting("db_name", "postgres")
            st.markdown(
                _done_badge(f"{db_user}@{db_host}:{db_port}/{db_name_display}"),
                unsafe_allow_html=True,
            )
            st.caption("")
            ...
            st.info(
                f"Configure the {RESOURCE_TYPE_LABEL} connection and OpenAI API key. "
            ...
            with st.form("db_config"):
                ...
                st.subheader("OpenAI")
                st.caption("Required for audio transcription (Whisper API)")
                ...
                if st.form_submit_button("Save Configuration", type="primary"):
                    ...
                    elif not openai_key or not openai_key.startswith("sk-"):
                        st.error("Valid OpenAI API key is required (starts with sk-)")
                    else:
                        try:
                            result = call_procedure(
                                "configure_database", db_host, db_port, db_user, db_pass, db_name, openai_key
                            )
                            if result.startswith("ERROR"):
                                ...
                                try:
                                    check_result = session.sql(
                                        "DESCRIBE SECRET app_config.db_credentials"
                                    ).collect()
                                    st.success("db_credentials secret verified.")
                                except Exception as e:
                                    st.error(f"db_credentials secret NOT found: {e}")
                                time.sleep(5)
                                st.rerun()
                        except Exception as e:
                            st.error(f"Exception calling configure_database: {e}")
    ...
    with st.expander(
        f"Approve EAI for {RESOURCE_TYPE_LABEL} & OpenAI connectivity",
        expanded=(s3 == "current"),
    ):
        if not step1_done or not step2_done:
            ...
            st.markdown(
                _done_badge(f"EAI Approved — {RESOURCE_TYPE_LABEL} & OpenAI access enabled"),
                unsafe_allow_html=True,
            )
        elif eai_stale:
            st.error(
            ...
            st.markdown(
                "**How to re-approve:**\n\n"
                "1. Click the app name in the top navigation bar to open app settings\n"
                f"2. Find **\"{EAI_DISPLAY_LABEL}\"** listed under **External Access**\n"
                "3. Click **Review** to verify the connection details:\n"
                f"   - Allowed hosts: `{db_host_display}:{db_port_display}`, `api.openai.com:443`\n"
                "4. Click **Approve** to activate the integration\n"
                "5. Return to this Setup page and click **Check EAI Status** below"
            )
        else:
            st.warning(
                f"**EAI approval is required** before the service can connect to "
                f"{RESOURCE_TYPE_LABEL} and OpenAI API.\n\n"
            ...
            st.markdown(
                ...
                "and OpenAI API for audio transcription."
            )
    ...
    with st.expander("Service status and Web UI", expanded=(s4 == "current")):
        if not step1_done or not step2_done or not step3_done:
            ...
            st.caption("The service is suspended. Click Recreate Service to restart.")
        else:
            st.info("**Create the service for the first time.**")
...
# ============================================================
    ...
    st.info("Configure service resources and view logs.")
